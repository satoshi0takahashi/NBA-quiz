function answer(){
  ...
  sending.addEventListener("click", function(){
    ...
    if (src == `assets/img1.png`){
      exp.innerHTML= "歴代最高峰の選手。３つの違うチームを引っ張りながら優勝を経験している。歴代の記録を数々更新している。恵まれた体格からインサイドにドライブして、ファウルをもらいながらAnd 1を決める。";
    }else if (src == `assets/img2.png`){
      exp.innerHTML= "歴代最高峰のシューター。GSWで時代を築いた。あのMicheal Jordanを超える７３勝９敗というシーズン勝利数を記録した。３pラインの遥か後ろからシュートを決める。新しい時代を確信させた選手。";
    }else if (src == `assets/img3.png`){
      exp.innerHTML= "LAL一筋で優勝経験５回を経験したSG。極端に負けず嫌いなことで有名な選手であり、「マンバメンタリティ」と呼ばれている。４２歳の若さで亡くなってしまったが、歴代２位である１試合８１得点の記録は現代バスケにおいては未だ到達していない。";
    }else if (src == `assets/img4.png`){
      exp.innerHTML= "CHIで長年いぶし銀な活躍をしていた選手。堅実なプレイメイクで定評があった。スーパースターではないが、チームを安定感をもたらしていた。";
    }else if (src == `assets/img5.png`){
      exp.innerHTML= "２年連続シーズンMVPを受賞している若きスーパースター。ギリシャ出身とかけて「Greek Freak」と呼ばれる。身長、手足が極端に長く、コートを凄いスピードで駆け抜ける。最近は苦手だったアウトサイドシュートも年々改善されて、完璧な選手にどんどん近づいている。";
    }else if (src == `assets/img6.png`){
      exp.innerHTML= "日本最高の選手。WASから１巡目８位指名を受けて晴れてNBA入り。安定したミドルレンジシュートを武器にインサイドを主戦場としている。個人的にはDEFを学んでよりブレイクして欲しい。";
    }else if (src == `assets/img7.png`){
      exp.innerHTML= "ジョージワシントン大学でエースとして活躍後、Twoーway契約を勝ち取ってNBAで活躍するチャンスを伺っている。ウイングスパンが長く、３＆Dでスマートなプレイを得意としている。今後の活躍に期待。";
  }
  })
}
